...
define(BLOCKLIST_VERSION, '2.2.2');

# This script only applies to editing (or the commentbox cookbook script), so 
# if we're not editing or commenting, just return.
if (!($action == 'edit' || $action == 'comment') ) return;

// Set default for $BlocklistPages to Site/Blocklist OR Main/Blocklist (fallback) if not custom
...
//old set default
//SDV($BlocklistPages, 'Main.Blocklist');
...
foreach((array)$BlocklistPages as $oneblockpage) {
  ...
  // if the current page is the blocklist page, don't check it!
  if ($pagename == $pn) return;

  // if the page does not exist, move to the next page
  if (!$page) continue;
  ...
  if (preg_match("/(\\D$ipa\\.$ipb\\.(\\D|$ipc)\\.($ipd)?\\D)/", $page['text'], $matchedip)) {
  	...
  	//catch and fix multiple block:'s on a single line and leading whitespace (allows legacy format)
    ...
    // grab the matches & trim them
    array_walk($blockterms[1], 'BlocklistFindBlocked');
...
if ($Blocklisted) { 
  ...
  if ($EnableWhyBlocked == "1") $EditMessageFmt .= "<pre>" . $WhyBlockedFmt . "</pre>";
}
